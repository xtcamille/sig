...
pub fn verify_signature_flow(
    ...
    shared_lib::verify_public_values(pub_key, message, signature, public_values)
        .map_err(|e| format!("Public values mismatch: {}", e))?;
    ...
    let pv_digest = hash_public_values(public_values);
    ...
    let inputs = [
        Fr::from(BigUint::from_bytes_be(&vkey_bytes)),
